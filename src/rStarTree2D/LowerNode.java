...
public class LowerNode implements Comparator<Object>, SuperNode {
	...
	public LowerNode(double x1, double x2, double y1, double y2,
			int mPartition, DistMatrix mDistMatrix, ConnectivityTier mConnectivityTier) {
		this.x1 = x1;
		this.y1 = y1;
		this.x2 = x2;
		this.y2 = y2;
	...
	public void updatemKeywords() {
		this.mKeywords = new int[DataGenConstant.mKeyworSize];
		mKeywords = IndoorSpace.iPartitions.get(this.mPartition).getmKeywords();
		
		int keywordSize = mKeywords.length;
		for (int i = 0; i < keywordSize; i ++) {
			if (this.mKeywords[i] >= 2 || this.mKeywords[i] < 0) System.out.println("something wrong_Block_updatemKeywordArr");
		}
	}
